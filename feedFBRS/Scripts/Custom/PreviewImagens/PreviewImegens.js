...
function atualizarPreview() {
    ...
    let elementsArray = []; // Temporary array to store elements before appending
    ...
    imagens.forEach((file, index) => {
        ...
        reader.onload = function (e) {
            ...
            imgWrapper.setAttribute("data-index", index); // Store index for sorting
            ...
            elementsArray[index] = imgWrapper; // Store element in the correct index
            if (elementsArray.length === imagens.length) {
                // Append elements in the correct order
                elementsArray.forEach(el => {
                    if (el) imagePreviewContainer.appendChild(el);
                });
            }
        };
...
// DRAG AND DROP
// ✅ Initialize Sortable only once, outside atualizarPreview()
const sortable = new Sortable(imagePreviewContainer, {
    animation: 150,
    onEnd: function (evt) {
        let newOrder = [];
        let elements = imagePreviewContainer.querySelectorAll(".image-container");

        elements.forEach((el, newIndex) => {
            let index = el.getAttribute("data-index");
            newOrder.push(imagens[index]); // Reorder array
            el.setAttribute("data-index", newIndex); // Update index in DOM
        });

        imagens = newOrder;
        atualizarPreview(); // Refresh UI with correct order
    }
});
...
function removerImagem(index) {
    ...
    // Update input file to reflect remaining images
